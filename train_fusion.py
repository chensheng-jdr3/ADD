...
# ==========================================
class ASF(nn.Module):
    def __init__(self, in_dim=2048, hidden_dim=256, cls_num=4, beta_shift=0.1, dropout_prob=0.2):
        super(ASF, self).__init__()
        self.beta_shift = beta_shift
        
        # 特征降维至统一空间
        self.proj_t = nn.Linear(in_dim, hidden_dim)
        self.proj_s = nn.Linear(in_dim, hidden_dim)
        
        # 门控与位移特征计算权重映射
        self.W_hav = nn.Linear(hidden_dim * 2, hidden_dim)
        self.W_av = nn.Linear(hidden_dim, hidden_dim)
    ...
    def forward(self, t_feat, s_feat):
        # 1. 独立映射
        t_embed = self.proj_t(t_feat)
        s_embed = self.dropout(self.proj_s(s_feat))
        ...
        fused_output = self.dropout(self.LayerNorm(shifted_embed + t_embed))
...
def evaluate_model(model_nbi, model_wli, fusion_net, dataloader, device):
    ...
    with torch.no_grad():
        for wli_img, nbi_img, label in tqdm(dataloader, desc="[Evaluate]", leave=False):
            ...
            logits = fusion_net(t_feat, s_feat)
    ...
    return acc, macro_pre, macro_rec, macro_f1
...
def main(opt):
    ...
    fusion_net = ASF(in_dim=2048, hidden_dim=opt.hidden_dim, cls_num=len(val_ds.class_map), beta_shift=opt.beta_shift).to(device)
    criterion = nn.CrossEntropyLoss()
    optimizer = torch.optim.AdamW(fusion_net.parameters(), lr=opt.lr, weight_decay=1e-4)
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        for wli_img, nbi_img, label in pbar:
        ...
        val_acc, val_pre, val_rec, val_f1 = evaluate_model(model_nbi, model_wli, fusion_net, val_loader, device)
        ...
        logging_msg = f"Epoch {epoch:02d} | Train L: {epoch_loss:.4f}, Acc: {epoch_acc:.4f} | Val Acc: {val_acc:.4f}, F1: {val_f1:.4f}"
...
if __name__ == '__main__':
    ...
    parser.add_argument('--nbi_ckpt', default='./pretrained/0_teacher.pth', help='NBI Teacher weights')
    parser.add_argument('--wli_ckpt', default='./pretrained/0_student.pth', help='WLI Student weights')
    parser.add_argument('--save_dir', default=f'./log/fusion_asf_{datetime.now().strftime("%Y%m%d_%H%M")}')
    parser.add_argument('--device', default='cuda:0')
    parser.add_argument('--epochs', type=int, default=30)
    parser.add_argument('--batch_size', type=int, default=16)
    parser.add_argument('--lr', type=float, default=5e-4) # AdamW default tuning rate
